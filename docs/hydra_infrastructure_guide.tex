...
\begin{tikzpicture}[
    ...
    server/.style={rectangle, draw, thick, fill=blue!20, minimum width=4cm, minimum height=2.5cm, align=center},
    gpu/.style={rectangle, draw, thick, fill=gpu-green!30, minimum width=4cm, minimum height=2.5cm, align=center},
    service/.style={rectangle, draw, fill=green!10, minimum width=2cm, minimum height=0.6cm, align=center, font=\scriptsize},
    container/.style={rectangle, draw, fill=orange!10, minimum width=1.8cm, minimum height=0.6cm, align=center, font=\scriptsize},
    arrow/.style={->, thick},
...
\node[box, right=2.5cm of user] (azure) {Azure AD\\SAML IdP};

% Traefik
\node[box, below=1.5cm of user] (traefik) {Traefik\\:80, :443};

% Hydra Server
\node[server, below left=2cm and 0cm of traefik] (hydra) {\textbf{Hydra}\\192.168.1.160\\Control Plane};
\node[service, below=0.3cm of hydra.north] (hydra-app) {hydra-saml-auth};
\node[container, below=0.1cm of hydra-app] (student1) {student-*};
\node[service, below=0.1cm of student1] (zfs) {ZFS 21TB};

% Chimera Server
\node[gpu, right=1.5cm of hydra] (chimera) {\textbf{Chimera}\\192.168.1.150\\3x RTX 3090};
\node[service, below=0.3cm of chimera.north] (openwebui) {OpenWebUI};
\node[service, below=0.1cm of openwebui] (ollama) {Ollama};

% Cerberus Server
\node[gpu, right=1.5cm of chimera] (cerberus) {\textbf{Cerberus}\\192.168.1.242\\2x RTX 5090};
\node[service, below=0.3cm of cerberus.north] (metrics) {metrics-agent};
\node[container, below=0.1cm of metrics] (gpu-student) {GPU containers};

% Storage
\node[box, below=1cm of hydra] (db) {SQLite\\Database};
\node[box, below=1cm of chimera] (nfs) {NFS\\Storage};
...
\draw[dashed_arrow] (hydra) -- node[below, font=\scriptsize] {NFS} (cerberus);
