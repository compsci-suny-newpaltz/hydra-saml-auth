...
\subsection{Service Approval Gating}

Each student container runs three managed services via \texttt{supervisord}. Code-server starts automatically; Jupyter and Jenkins require admin approval before they can be started.
...
\midrule
code-server & 8443 & \texttt{/students/\{user\}/vscode/} & Yes & None required \\
Jupyter Lab & 8888 & \texttt{/students/\{user\}/jupyter/} & No & \texttt{jupyter\_execution\_approved} \\
Jenkins & 8080 & \texttt{/students/\{user\}/jenkins/} & No & \texttt{jenkins\_execution\_approved} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[nosep]
    \item Student clicks ``Request Access'' on the dashboard
    \item Request stored in \texttt{resource\_requests} table (type: \texttt{jupyter\_execution} or \texttt{jenkins\_execution})
    \item Admin approves via admin panel, setting the flag in \texttt{user\_quotas}
    \item On next container init, the \texttt{JUPYTER\_APPROVED} / \texttt{JENKINS\_APPROVED} env var is injected
    \item \texttt{entrypoint.sh} creates \texttt{/var/run/jupyter-approved} or \texttt{/var/run/jenkins-approved} marker
    \item Supervisor starts the approved service
\end{enumerate}
